...
app.post('/verify/action/:verificationId', async (req, res) => {
    ...
    // Busca o objeto de usuário do admin para passar para a função de processamento
    const adminUser = await client.users.fetch(process.env.OWNER_ID);
    const result = await processVerificationAction(finalAction, tokenData.context, adminUser);
