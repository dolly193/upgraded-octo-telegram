...
function createShoppingPanel(productStock) {
    ...
    const productOptions = inStockProducts.map(([productId, product]) => {
        ...
        return {
            label: product.name.slice(0, 100), // CORREÇÃO: Limita o nome a 100 caracteres
...
async function processVerificationAction(action, context, adminUser) {
    ...
    if (action === 'approve') {
        if (type === 'ticket') {
            return await processPaymentConfirmation(guild, channelId, productId, adminUser);
        } else if (type === 'site') {
...
client.on(Events.MessageCreate, async (message) => {
    ...
    if (command === 'painel') {
        if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            return message.reply('❌ Você não tem permissão para usar este comando.').then(msg => setTimeout(() => msg.delete(), 5000));
        }
        ...
        const panelMessage = await message.channel.send(panel);

        // Salva as informações do painel fixo
        await setConfig('fixed_panel', { panelMessageId: panelMessage.id, panelChannelId: panelMessage.channel.id });
        await logAuditEvent(`Painel de compras fixo definido no canal #${panelMessage.channel.name} por ${message.author.tag}.`);

        // Deleta a mensagem de comando original para manter o canal limpo
        await message.delete();
    }
});
